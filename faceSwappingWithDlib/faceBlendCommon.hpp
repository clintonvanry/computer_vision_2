...
void constrainPoint(Point2f &p, const Size& sz)
{
...
void getEightBoundaryPoints(const Size& size, vector<Point2f>& boundaryPts)
{
  int h = size.height, w = size.width;
  boundaryPts.emplace_back(0,0);
  boundaryPts.emplace_back(w/2, 0);
  boundaryPts.emplace_back(w-1,0);
  boundaryPts.emplace_back(w-1, h/2);
  boundaryPts.emplace_back(w-1, h-1);
  boundaryPts.emplace_back(w/2, h-1);
  boundaryPts.emplace_back(0, h-1);
  boundaryPts.emplace_back(0, h/2);
}
...
static void calculateDelaunayTriangles(const Rect& rect, vector<Point2f> &points, vector< vector<int> > &delaunayTri){
...
void warpTriangle(Mat &img1, Mat &img2, vector<Point2f> t1, vector<Point2f> t2)
{
  ...
  for(int i = 0; i < 3; i++)
  {
    ...
    t2RectInt.emplace_back((int)(t2[i].x - r2.x), (int)(t2[i].y - r2.y) ); // for fillConvexPoly
...
vector<Point2f> getLandmarks(dlib::frontal_face_detector &faceDetector, dlib::shape_predictor &landmarkDetector, Mat &img, float FACE_DOWNSAMPLE_RATIO = 1 )
{
  ...
  std::cout << "Detect faces in the image " << std::endl;
  ...
  if(!faceRects.empty())
  {
    ...
    std::cout << "dlib::full_object_detection " << std::endl;
