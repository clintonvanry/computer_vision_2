...
void constrainRect(Rect &r, const Size& sz)
{
    if(r.x < 0)
        r.x = 0;
...
int main() {
    ...
    {
        // Grab a frame
        auto time_total = (double)cv::getTickCount();
        cap >> src;
        int height = src.rows;
        ...
        std::cout << "Time taken by detector at scale " << FACE_DOWNSAMPLE_RATIO_DLIB << " = "<< ((double)cv::getTickCount() - time_total)/cv::getTickFrequency() << std::endl;
        ...
        std::cout << "Total Time taken at scale " << FACE_DOWNSAMPLE_RATIO_DLIB << " = "<< ((double)cv::getTickCount() - time_total)/cv::getTickFrequency() << std::endl;
